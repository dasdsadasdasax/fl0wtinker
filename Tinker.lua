...
RefreshTime = 0
local AbilityChannelTime = {3, 1.5, 0.75}
...
function Tinker.OnUpdate()
    ...
    if Menu.IsKeyDown(Tinker.menuComboKey) then
    	if CurrentVictim and GameRules.GetGameTime() - RefreshTime >= 0.1 then
        	if not NPC.HasModifier(MyHero, "modifier_tinker_rearm") then	
         		Tinker.DoCombo()
         	end 
     	end
    end
...
function Tinker.DoCombo()
    ...
    then
        if Tinker.CastAbility(SpellRefresh, Tinker.menuAbilitiesHandle["SpellRefresh"]) then 
            NextRefreshTime = GameRules.GetGameTime() + Ability.GetCastPoint(SpellRefresh) + AbilityChannelTime[Ability.GetLevel(SpellRefresh)]
            RefreshTime = GameRules.GetGameTime() + Ability.GetCastPoint(SpellRefresh) + 0.1
            return 
        end
    end
end
...
function Tinker.EnemySelector()
    ...
    if selector_mode == 0 and not temp_enemy then
        ...
        if temp_enemy and Utility.CanCastSpellOn(temp_enemy) and NPC.IsPositionInRange(temp_enemy, Input.GetWorldCursorPos(), mouse_range) then
